...
impl Playfield {
    ...
    fn possible_moves(&self) -> Vec<Move> {
        ...
        for amphipod in &self.amphipods {
            ...
            if (position.y == 0 && position.x == 1 && self.grid[0][2] != '.')
                || (position.y == 0 && position.x == 11 && self.grid[0][10] != '.')
            {
                continue;
            }
            ...
            if position.x == target_x
                && self.grid[position.y..self.grid.len()]
                    .iter()
                    .map(|r| r[position.x])
                    .filter(|c| c != &name)
                    .collect::<Vec<char>>()
    ...
    fn check_path(&self, start: &Position, end: &Position) -> u64 {
        ...
        loop {
            if updated_position.x != end.x {
                ...
                // target x reached
                if updated_position.y < self.grid.len()
                    && self.grid[updated_position.y + 1][updated_position.x] == '.'
                {
    ...
    fn generate_amphipods(grid: &Vec<Vec<char>>) -> Vec<Amphipod> {
        ...
        for y in 0..grid.len() {
            for x in 0..grid[0].len() {
                match grid[y][x] {
                    'A' | 'B' | 'C' | 'D' => {
                        amiphods.push(Amphipod::new(grid[y][x], Position::new(x, y)))
                    }
                    _ => {}
                }
            }
...
fn main() {
    ...
    loop {
        ...
        for playfield in playfields {
            ...
            let finished: Vec<Playfield> = new_candidates
                .iter()
                .filter(|p| p.is_done())
                .map(|p| p.clone())
                .collect();
...
mod tests {
    ...
    fn test_part2() {
        ...
        assert_eq!(
            new_playfields.iter().map(|p| p.total_energy).max().unwrap(),
            900
        );
    }
    ...
    fn test_part_calc() {
        ...
        // }
        assert_eq!(
            playfield.check_path(&Position { x: 9, y: 1 }, &Position { x: 11, y: 0 }),
            3
        );
        ...
        assert_eq!(
            playfield.check_path(&Position { x: 9, y: 2 }, &Position { x: 1, y: 0 }),
            10
        );
        playfield = playfield.apply_move(Move {
        ...
        assert_eq!(
            playfield.check_path(&Position { x: 7, y: 1 }, &Position { x: 10, y: 0 }),
            4
        );
        playfield = playfield.apply_move(Move {
        ...
        assert_eq!(
            playfield.check_path(&Position { x: 7, y: 2 }, &Position { x: 8, y: 0 }),
            3
        );
        playfield = playfield.apply_move(Move {
        ...
        assert_eq!(
            playfield.check_path(&Position { x: 7, y: 3 }, &Position { x: 2, y: 0 }),
            8
        );
        playfield = playfield.apply_move(Move {
        ...
        assert_eq!(
            playfield.check_path(&Position { x: 5, y: 1 }, &Position { x: 7, y: 3 }),
            6
        );
        playfield = playfield.apply_move(Move {
