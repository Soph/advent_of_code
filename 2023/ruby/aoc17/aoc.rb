...
while !moves.empty?
  ...
  directions.each do |direction|
    ...
    if direction == move[1] # continue same direction
      ...
      if steps_direction >= 3
        steps_direction = 0
      else
        next
      end
    end
    next if steps_direction >= 10 # to much, not possible
    #puts "      #{move[3]} + #{grid[new_pos[1]][new_pos[0]]}"
    new_heat = move[3] + grid[new_pos[1]][new_pos[0]]
    next if new_heat > min_finish_heat
    if new_pos == [grid[0].size - 1, grid.size - 1]
      next if steps_direction < 3
      # finish
      ...
      #puts "#{move[0]},#{move[3]} -> #{[new_pos, direction, steps_direction, new_heat, move[4]+1].inspect}"
      #sleep 1
