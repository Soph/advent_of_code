...
fn bags_contained_count(bags: &HashMap<String, HashSet<String>>, name: &String, result: &mut HashSet<String>) {
    match bags.get(name) {
        None => {
    ...
    for row in contents.split("\n") {
        ...
        if bag.name != *main_bag {
            for name in bag.bags {
                bag_mapping.entry(name).or_insert(HashSet::new()).insert(bag.name.clone());
            }            
        }
    }
...
fn parse_bag(data: &str) -> Bag {
    ...
    if parts[1] == "no other bag" {
        return Bag { name: bag_name.clone(), bags: vec![] };
    }

    let contains: Vec<String> = parts[1].replace(".", "").split(", ").map(|s| normalize_bag_name(&s[2..s.len()].to_string())).collect();

    println!("{} contains {}", bag_name, contains.join(","));
    Bag { name: bag_name.clone(), bags: contains }
}
