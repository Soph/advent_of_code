...
fn calculate_all(line: &String, calculate_numbers:fn (&String) -> u64) -> u64 {
    ...
    loop {
        ...
        for capture in regex.captures_iter(processing.as_str()) {
            found = true;
            replacements.insert(capture[0].to_string(), calculate_numbers(&capture[0].replace("(","").replace(")","").to_string()));
        }
        ...
        match captures {
            Some(capture) => {
                match &capture[2] {
                    "+" => {
                        found = true;
                        replacements.insert(capture[0].to_string(), capture[1].parse::<u64>().unwrap() + capture[3].parse::<u64>().unwrap());
                    },
                    "*" => {
                        found = true;
                        replacements.insert(capture[0].to_string(), capture[1].parse::<u64>().unwrap() * capture[3].parse::<u64>().unwrap());
...
mod tests {
    ...
    #[test]
    fn test_calculate2() {
        assert_eq!(calculate_numbers2(&"1 + 2 * 3 + 4 * 5 + 6".to_string()), 231);
    ...
    fn test_calculate_all() {
        assert_eq!(calculate_all(&"7 * 7 * (5 + 9 + 9) * (8 * 6 * 5 + 4 * 2)".to_string(), calculate_numbers1), 549976);
        ...
        assert_eq!(calculate_all(&"4 + 5 + 6 * 6 * 6 + (3 * 2 * 3 * (2 * 6 + 9) + 6 + (5 + 2 + 9 * 6))".to_string(), calculate_numbers1), 1020);
    }

    #[test]
    fn test_calculate_all2() {
        assert_eq!(calculate_all(&"7 * 7 * (5 + 9 + 9) * (8 * 6 * 5 + 4 * 2)".to_string(), calculate_numbers2), 549976);
        ...
        assert_eq!(calculate_all(&"4 + 5 + 6 * 6 * 6 + (3 * 2 * 3 * (2 * 6 + 9) + 6 + (5 + 2 + 9 * 6))".to_string(), calculate_numbers2), 1020);
    }
